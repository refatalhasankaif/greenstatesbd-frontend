...
function getStoredUser(): IUser | null {
  ...
  try {
    ...
    // ✅ Discard stale/corrupt data that's missing required fields
...
function Avatar({ user, size = 32 }: { user: IUser; size?: number }) {
  ...
  if (user.profileImage) {
    return (
      <Image src={user.profileImage} alt={user.name ?? "User"} width={size} height={size}
...
const Navbar = ({ className }: { className?: string }) => {
  ...
  const menu = [
    ...
    {
      title: "Explore", icon: <Compass size={16} />,
      items: [
    ...
    {
      title: "AI", icon: <Sparkles size={16} />,
      items: [
              ...
              item.items ? (
                <div key={item.title} className="relative">
                  ...
                  >
                    ...
                    <ChevronDown size={13} className={cn("transition-transform duration-200", openMenu === item.title && "rotate-180")} />
                  </button>
                  {openMenu === item.title && (
                    <div className="absolute left-0 top-full mt-2 w-52 rounded-lg border bg-popover shadow-lg p-1.5 z-50">
                      {item.items.map((sub) => (
                        <Link key={sub.title} href={sub.url} onClick={() => setOpenMenu(null)}
          ...
          {user ? (
            <div className="relative" ref={profileRef}>
              ...
              >
                ...
                <ChevronDown size={13} className={cn("transition-transform duration-200 text-muted-foreground", profileOpen && "rotate-180")} />
              </button>
                    ...
                    {item.items ? (
                      <>
                        ...
                        >
                          ...
                          <ChevronDown size={13} className={cn("transition-transform duration-200", mobileOpen === item.title && "rotate-180")} />
                        </button>
